import { calculator, capitalize, reverseString , analyzeArray ,caesarCipher} from "./functions";
...
test('analyzeArray sum: 15, max: 5, average: 3, length: 5, min: 1', () => {
    ...
    expect(analyzeArray(numbers)).toEqual({sum: 15, max: 5, average: 3, length: 5, min: 1});
  });

test('ceasarCipher return "Ymj Tins uwtojhy Wzqjx!!" on input: "The Odin project Rules!!", 5 ', () => {
    const numbers = [1, 2, 3, 4, 5]
    expect(caesarCipher("The Odin project Rules!!", 5)).toBe("Ymj Tins uwtojhy Wzqjx!!");
  });
